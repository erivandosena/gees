...
uses
  ...
  ExtCtrls, StdCtrls, Buttons, Registry, ComCtrls, Mask, DB,
...
type
  TFrmSenha = class(TForm)
    ...
    EditSenha: TEdit;
    IBQ_Senhas: TIBQuery;
    IBQ_SenhasNOME_USUARIO: TIBStringField;
    IBQ_SenhasSENHA: TIBStringField;
    DSSENHA: TDataSource;
...
IBQ_Senhas.close;
IBQ_Senhas.sql.clear;
IBQ_Senhas.sql.add('select * from USUARIOS where NOME_USUARIO='+char(39)+EditNome.text+char(39)+'and SENHA='+char(39)+Codificado+char(39));
IBQ_Senhas.open;                                                                                               //      +EditSenha.text+char(39));
if (IBQ_SenhasNOME_USUARIO.Value<>'') and (IBQ_SenhasSENHA.Value<>'')then
begin
IBQ_Senhas.close;
IBQ_Senhas.sql.clear;
IBQ_Senhas.sql.add('select * from USUARIOS');
IBQ_Senhas.open;
IBQ_Senhas.Locate('NOME_USUARIO', EditNome.Text, []);
iOK := 625;
...
Reg.OpenKey('GEES\Login', True);
Verificar:=Reg.ReadString('Nome');
...
Reg:=TRegistry.Create;
Reg.CreateKey('GEES\Login');
If Reg.OpenKey('GEES\Login', True) then
begin
Reg.WriteString('Nome', Caminho);
Reg.CloseKey;
end;
end;
